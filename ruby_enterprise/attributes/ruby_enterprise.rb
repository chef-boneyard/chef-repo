...
when 'debian','ubuntu'
    ...
    default[:ruby_enterprise][:url][:i386] = "#{node[:opsworks_commons][:assets_url]}/packages/#{node[:platform]}/12.04/ruby-enterprise_1.8.7-#{ruby_enterprise[:version]}_i386_ubuntu10.04.deb"
    default[:ruby_enterprise][:url][:amd64] = "#{node[:opsworks_commons][:assets_url]}/packages/#{node[:platform]}/12.04/ruby-enterprise_1.8.7-#{ruby_enterprise[:version]}_amd64_ubuntu10.04.deb"
